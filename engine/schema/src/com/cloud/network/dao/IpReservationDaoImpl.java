...
public class IpReservationDaoImpl extends GenericDaoBase<IpReservationVO, Long> implements IpReservationDao {
    ...
    @Override
    public List<IpReservationWithNetwork> getAllIpReservations() {
        try (TransactionLegacy txn = TransactionLegacy.currentTxn()) {
            PreparedStatement pstmt = txn.prepareAutoCloseStatement("SELECT r.uuid, r.start_ip, r.end_ip, n.uuid from ip_reservation r " +
                    "INNER JOIN networks n on n.id = r.network_id where r.removed is null");
            ResultSet rs = pstmt.executeQuery();
            List<IpReservationWithNetwork> ret = new ArrayList<>();
            while (rs.next()) {
                ret.add(new IpReservationWithNetwork(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4)));
            }
            return ret;
