...
"error.could.not.change.your.password.because.non.native.user":"Error could not change your password because user is not a native Cloud Infrastructure user.",
...
"force.remove.host.warning":"Warning: Choosing this option will cause Cloud Infrastructure to forcefully stop all running virtual machines before removing this host from the cluster.",
...
"label.about":"About",
"label.about.app":"About Cloud Infrastructure",
...
"label.delete.volumes":"Delete data volumes",
"label.volume.empty":"No volumes attached to this VM",
"label.volume.ids":"Data volumes",
...
"label.app.name":"Cloud Infrastructure",
"label.apply":"Apply",
...
"label.health.check.message.desc":"Your load balancer will automatically perform health checks on your Cloud Infrastructure instances and only route traffic to instances that pass the health check",
...
"label.installWizard.subtitle":"This tour will aid you in setting up your Cloud Infrastructure&#8482 installation",
"label.installWizard.title":"Hello and Welcome to Cloud Infrastructure&#8482",
...
"label.introduction.to.Cloud Infrastructure":"Introduction to Cloud Infrastructure&#8482",
...
"label.remote.id": "Remote ID",
"label.remote.id.type": "Remote ID Type",
...
"label.service.provider.metadata":"SAML Service Provider Metadata",
...
"label.size":"Size",
"label.skip.guide":"I have used Cloud Infrastructure before, skip this guide",
...
"label.what.is.Cloud Infrastructure":"What is Cloud Infrastructure&#8482?",
...
"label.zoneWizard.trafficType.management":"Management: Traffic between Cloud Infrastructure's internal resources, including any components that communicate with the Management Server, such as hosts and Cloud Infrastructure system VMs",
...
"message.add.pod.during.zone.creation":"Each zone must contain in one or more pods, and we will add the first pod now. A pod contains hosts and primary storage servers, which you will add in a later step. First, configure a range of reserved IP addresses for Cloud Infrastructure's internal management traffic. The reserved IP range must be unique for each zone in the cloud.",
...
"message.assign.instance.another":"Please specify the account type, domain, account name and network (optional) of the new account. <br> If the default nic of the vm is on a shared network, Cloud Infrastructure will check if the network can be used by the new account if you do not specify one network. <br> If the default nic of the vm is on a isolated network, and the new account has more one isolated networks, you should specify one.",
...
"message.desc.create.ssh.key.pair":"Please fill in the following data to create or register a ssh key pair.<br><br>(1) If public key is set, Cloud Infrastructure will register the public key. You can use it through your private key.<br><br>(2) If public key is not set, Cloud Infrastructure will create a new SSH Key pair. In this case, please copy and save the private key. Cloud Infrastructure will not keep it.<br>",
"message.desc.created.ssh.key.pair":"Created a SSH Key Pair.",
"message.desc.host":"Each cluster must contain at least one host (computer) for guest VMs to run on, and we will add the first host now. For a host to function in Cloud Infrastructure, you must install hypervisor software on the host, assign an IP address to the host, and ensure the host is connected to the Cloud Infrastructure management server.<br/><br/>Give the host's DNS or IP address, the user name (usually root) and password, and any labels you use to categorize hosts.",
...
"message.desc.zone":"A zone is the largest organizational unit in Cloud Infrastructure, and it typically corresponds to a single datacenter. Zones provide physical isolation and redundancy. A zone consists of one or more pods (each of which contains hosts and primary storage servers) and a secondary storage server which is shared by all pods in the zone.",
...
"message.guest.traffic.in.basic.zone":"Guest network traffic is communication between end-user virtual machines. Specify a range of IP addresses that Cloud Infrastructure can assign to guest VMs. Make sure this range does not overlap the reserved system IP range.",
...
"message.installWizard.copy.whatIsACluster":"A cluster provides a way to group hosts. The hosts in a cluster all have identical hardware, run the same hypervisor, are on the same subnet, and access the same shared storage. Virtual machine instances (VMs) can be live-migrated from one host to another within the same cluster, without interrupting service to the user. A cluster is the third-largest organizational unit within a Cloud Infrastructure&#8482; deployment. Clusters are contained within pods, and pods are contained within zones.<br/><br/>Cloud Infrastructure&#8482; allows multiple clusters in a cloud deployment, but for a Basic Installation, we only need one cluster.",
"message.installWizard.copy.whatIsAHost":"A host is a single computer. Hosts provide the computing resources that run the guest virtual machines. Each host has hypervisor software installed on it to manage the guest VMs (except for bare metal hosts, which are a special case discussed in the Advanced Installation Guide). For example, a Linux KVM-enabled server, a Citrix XenServer server, and an ESXi server are hosts. In a Basic Installation, we use a single host running XenServer or KVM.<br/><br/>The host is the smallest organizational unit within a Cloud Infrastructure&#8482; deployment. Hosts are contained within clusters, clusters are contained within pods, and pods are contained within zones.",
"message.installWizard.copy.whatIsAPod":"A pod often represents a single rack. Hosts in the same pod are in the same subnet.<br/><br/>A pod is the second-largest organizational unit within a Cloud Infrastructure&#8482; deployment. Pods are contained within zones. Each zone can contain one or more pods; in the Basic Installation, you will have just one pod in your zone.",
"message.installWizard.copy.whatIsAZone":"A zone is the largest organizational unit within a Cloud Infrastructure&#8482; deployment. A zone typically corresponds to a single datacenter, although it is permissible to have multiple zones in a datacenter. The benefit of organizing infrastructure into zones is to provide physical isolation and redundancy. For example, each zone can have its own power supply and network uplink, and the zones can be widely separated geographically (though this is not required).",
"message.installWizard.copy.whatIsCloud Infrastructure":"Cloud Infrastructure&#8482 is a software platform that pools computing resources to build public, private, and hybrid Infrastructure as a Service (IaaS) clouds. Cloud Infrastructure&#8482 manages the network, storage, and compute nodes that make up a cloud infrastructure. Use Cloud Infrastructure&#8482 to deploy, manage, and configure cloud computing environments.<br/><br/>Extending beyond individual virtual machine images running on commodity hardware, Cloud Infrastructure&#8482 provides a turnkey cloud infrastructure software stack for delivering virtual datacenters as a service - delivering all of the essential components to build, deploy, and manage multi-tier and multi-tenant cloud applications. Both open-source and Premium versions are available, with the open-source version offering nearly identical features.",
"message.installWizard.copy.whatIsPrimaryStorage":"A Cloud Infrastructure&#8482; cloud infrastructure makes use of two types of storage: primary storage and secondary storage. Both of these can be iSCSI or NFS servers, or localdisk.<br/><br/><strong>Primary storage</strong> is associated with a cluster, and it stores the disk volumes of each guest VM for all the VMs running on hosts in that cluster. The primary storage server is typically located close to the hosts.",
...
"message.installWizard.tooltip.addCluster.name":"A name for the cluster.  This can be text of your choosing and is not used by Cloud Infrastructure.",
...
"message.installWizard.tooltip.addPod.reservedSystemEndIp":"This is the IP range in the private network that the Cloud Infrastructure uses to manage Secondary Storage VMs and Console Proxy VMs. These IP addresses are taken from the same subnet as computing servers.",
...
"message.installWizard.tooltip.addPod.reservedSystemStartIp":"This is the IP range in the private network that the Cloud Infrastructure uses to manage Secondary Storage VMs and Console Proxy VMs. These IP addresses are taken from the same subnet as computing servers.",
...
"message.public.traffic.in.advanced.zone":"Public traffic is generated when VMs in the cloud access the internet. Publicly-accessible IPs must be allocated for this purpose. End users can use the Cloud Infrastructure UI to acquire these IPs to implement NAT between their guest network and their public network.<br/><br/>Provide at least one range of IP addresses for internet traffic.",
"message.public.traffic.in.basic.zone":"Public traffic is generated when VMs in the cloud access the Internet or provide services to clients over the Internet. Publicly accessible IPs must be allocated for this purpose. When a instance is created, an IP from this set of Public IPs will be allocated to the instance in addition to the guest IP address. Static 1-1 NAT will be set up automatically between the public IP and the guest IP. End users can also use the Cloud Infrastructure UI to acquire additional IPs to implement static NAT between their instances and the public IP.",
...
"message.storage.traffic":"Traffic between Cloud Infrastructure's internal resources, including any components that communicate with the Management Server, such as hosts and Cloud Infrastructure system VMs. Please configure storage traffic here.",
...
"message.tooltip.internal.dns.1":"Name of a DNS server for use by Cloud Infrastructure internal system VMs in the zone. The private IP address for the pods must have a route to this server.",
"message.tooltip.internal.dns.2":"Name of a DNS server for use by Cloud Infrastructure internal system VMs in the zone. The private IP address for the pods must have a route to this server.",
...
"message.your.Cloud Infrastructure.is.ready":"Your Cloud Infrastructure is ready!",
