...
@APICommand(name = "impersonateUser", description = "Logs a user into the CloudStack. A successful login attempt will generate a JSESSIONID cookie value that can be passed in subsequent Query command calls until the \"logout\" command has been issued or the session has expired.", requestHasSensitiveInfo = true, responseObject = LoginCmdResponse.class, entityType = {})
public class ImpersonateUserCmd extends BaseCmd /*implements APIAuthenticator*/ {
    ...
    public void execute() throws ServerApiException {
        ...
        setResponseObject(_apiServer.impersonateUser(getSession(), username, domainId, domain, null, null));
//        return ApiResponseSerializer.toSerializedString(_apiServer.impersonateUser(session, username[0], domainId, domain, remoteAddress, params), responseType);
...
//                }
//                return ApiResponseSerializer.toSerializedString(_apiServer.impersonateUser(session, username[0], domainId, domain, remoteAddress, params),
//                        responseType);
